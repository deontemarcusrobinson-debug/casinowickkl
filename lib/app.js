...
// Render / load balancer health check — must stay before bot/ban middleware
app.get('/healthz', function(req, res) {
    res.status(200).type('text/plain').send('ok');
});

// Casino catalog status (safe — no secrets)
app.get('/casino/status', function(req, res) {
    try {
        var casinoService = require('@/services/games/casinoService.js');
        res.status(200).json(casinoService.getCasinoStatus());
    } catch (e) {
        res.status(200).json({ error: String(e && e.message ? e.message : e) });
    }
});

// OAuth/recaptcha readiness (safe — no secrets)
app.get('/auth/status', function(req, res) {
    try {
        var config = require('@/config/config.js');
        res.status(200).json({
            google: {
                active: !!(config.app.google && config.app.google.active),
                callback: config.app.google && config.app.google.callback_url
            },
            discord: {
                active: !!(config.app.discord && config.app.discord.active),
                callback: config.app.discord && config.app.discord.callback_url
            },
            recaptcha: {
                active: !!(config.app.recaptcha && config.app.recaptcha.active)
            }
        });
    } catch (e) {
        res.status(200).json({ error: String(e && e.message ? e.message : e) });
