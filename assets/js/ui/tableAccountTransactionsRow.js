function tableAccountTransactionsRow(edata) {
...
;
var __line = 1
  , __lines = "<div class=\"table-row <% if(amount < 0){ %>text-danger<% } else { %>text-success<% } %>\">\n    <div class=\"table-column text-left\">#<%= id %></div>\n    <div class=\"table-column text-left capitalize\"><%= transaction.split('_').join(' ') %></div>\n\n    <div class=\"table-column text-left\">\n        <div class=\"flex flex-row items-center justify-start gap-1\">\n            <%- icon({\n                icon: 'diamond',\n                id: null,\n                className: 'text-primary size-4'\n            }); %>\n\n            <span><%= amount %></span>\n        </div>\n    </div>\n\n    <div class=\"table-column text-left\"><%= date %></div>\n</div>"
...
try {
  ...
  with (locals || {}) {
    ...
    ; __line = 3
    ; __append(escapeFn( transaction.split('_').join(' ') ))
    ; __append("</div>\n\n    <div class=\"table-column text-left\">\n        <div class=\"flex flex-row items-center justify-start gap-1\">\n            ")
    ; __line = 7
    ; __append( icon({
                icon: 'diamond',
