function tableAdminSupportRequestsRow(edata) {
...
try {
  ...
  with (locals || {}) {
    ...
    ; __append("\n            ")
    ; __line = 10
    ; __append( userField({ user: assigned }) )
    ; __append("\n        ")
    ; __line = 11
    ;  } else if(closed) { 
    ; __append("\n            <div class=\"font-bold text-danger\">Unavailable</div>\n        ")
    ...
    ; __append("\n    </div>\n    <div class=\"table-column\">\n        <div class=\"flex flex-row justify-end\">\n            <div class=\"")
    ; __line = 19
    ; __append(escapeFn( [ { 0: 'bg-danger bg-opacity-50', 1: 'bg-warning bg-opacity-50', 2: 'bg-success bg-opacity-50' }[status], 'bg-info bg-opacity-50' ][closed] ))
    ; __append(" p-1 rounded-1\">")
    ; __append(escapeFn( [ { 0: 'Opened', 1: 'Unsolved', 2: 'Answered' }[status], 'Solved' ][closed] ))
    ; __append("</div>\n        </div>\n    </div>\n</div>")
    ; __line = 22
  }
  return __output;
